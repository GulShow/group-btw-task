...
describe('CommissionCalculator service tests', () => {
  it('Data from file', async () => {
    const calculator = new CommissionCalculator();
    const content = await calculator.readInput('./input.json');
    assert.deepEqual(content, transactionsList);
  });

  it('Transactions from file', async () => {
    const calculator = new CommissionCalculator();
    await calculator.readInput('./input.json');
    await calculator.getCommissionRules();
    const result = (await calculator.calculate()).map(
